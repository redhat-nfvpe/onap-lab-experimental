...
We can simultaneously set up yet another watch for the overcloud servers:

    watch hypervisor/tripleo/openstack server list

Note that the server names are *not* the same as the infrastructure node names. At some point the
servers will actually appear (with "baremetal" flavor"). Their initial status will be "BUILD" and
eventually switch to "ACTIVE" and they will receive network IP addresses.
